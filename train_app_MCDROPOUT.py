...
from network_instance import IResNetDropout
...
class TrainingApp:
    def __init__(self, sys_argv=None):
        ...
        parser.add_argument(
            "--mc_samples",
            help="Number of Monte Carlo dropout samples for validation",
            default=5,
            type=int,
        )
    ...
    def initModel(self):
        log.info("Loading IResNetDropout CNN...")

        model = IResNetDropout(p=self.cli_args.dropout_rate)
    ...
    def main(self):
        ...
        log.info(f"Dropout rate: {self.cli_args.dropout_rate}")
        log.info(f"Monte Carlo Samples: {self.cli_args.mc_samples}")
        ...
        for epoch_ndx in range(1, n_epoch + 1):
            ...
            # Keep the model in training mode for MC dropout
            self.model.train()
            ...
            with torch.no_grad():
                ...
                for val_set in val_dl:
                    ...
                    val_outputs = []
                    for _ in range(self.cli_args.mc_samples):
                        val_outputs.append(self.model(val_batch))
                    val_outputs = torch.stack(val_outputs, dim=0).mean(dim=0)
